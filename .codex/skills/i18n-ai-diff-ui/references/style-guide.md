...
- Place the title and inspection explanation at the top left. Keep `Scan project` at the top right.
...
- Table editor: keep `Save changes` as its only primary action. Draft, pending, key-missing, empty-string, skipped, conflict, AI draft, and failed translation states use small semantic markers on neutral table surfaces.
...
- File writes are never automatic: panel editing is available by default, changes remain a per-file browser draft, and conflicts must be resolved before saving.
...
- Surface table states with small fills/markers: Changed, Pending, Key missing, Empty string, Skipped, AI draft, Failed. Skipped cells must have a visible skipped background. Pending, Empty string, and AI states must remain distinguishable from ordinary Changed.
...
- Modal size may vary by intent: confirmation modals stay compact, global search can use the large size. The visual language—radius, padding, close button, title weight, action row, surface colors, and overlay treatment—must remain shared.
...
- Explorer status priority is `Invalid JSON` > `Missing language files` > `Key missing` > `Empty string` > `Pending keys` > clean. Directory headings summarize descendant status; file rows show concrete status/count.
...
- Global search is a modal. It searches copy across configured locale files, can optionally include key paths, and uses shadcn `Checkbox` controls for language/state filters.
...
- Master-to-master AI translation is available from a master language column-header context menu and from `Translate selected` when the current selection contains cells from one master language only. Use the same shared confirmation modal shell as selected-cell translation, show the source-master selector and overwrite/cache options inside the modal, and keep results as browser drafts until Save.
- CLI shortcut is a separate top-level page for cross-file CLI-equivalent runs. It must use the shared topbar, operation bar, bottom bar, bento card surfaces, shadcn `Dialog`/business modal shell, and black primary action. Direct execution must clearly state that it writes local files, cache, and source baselines immediately after confirmation rather than creating editor drafts.
- Settings is a separate top-level page for visual `i18n-translate.config.*` editing. It must use the shared topbar, operation bar, bottom bar, bento card surfaces, shared modal shell, shadcn-style `Input`/`Textarea` form primitives, and black primary `Save settings` action. Keep locale/cache paths and route definitions in `Language routes`; show the current `Multi-master` or `Single-master` mode as a non-interactive light-violet badge on the right side of that card header, not in the operation bar. In Simplified Chinese UI copy, these mode names must be `多母版模式` and `单母版模式`. Keep prompt, skip keys, concurrency, batch size, and CLI watch debounce/ignore patterns in `AI behavior`; do not expose or serialize a `watch.enabled` toggle because CLI watch remains an explicit `--watch` command behavior. Keep `Panel style` as the right-side panel-behavior card above `Model runtime`; it owns panel-only personal preferences such as panel language. Panel language must support English and Simplified Chinese, persist to browser localStorage, and never be serialized to project config. Keep `Model runtime` in the fourth card position as display-only runtime information. Treat config saves as explicit local writes gated by the session write token, same-origin checks, and revision checks. The page must communicate that saves patch managed fields in place rather than regenerating the full module; custom imports/helpers and the `llm` block are preserved. The page must not imply that changing config rewrites locale JSON, cache, or source baselines; after saving, clearly require restarting the panel for routes, paths, prompt, batch behavior, or watcher changes to apply.
- Panel UI i18n must dogfood this project's recommended locale organization: dictionaries live in `panel/src/i18n/locales/<locale>/panel.json`, provider/hook exports live in `panel/src/i18n/index.tsx`, and browser preference/formatting helpers live in `panel/src/i18n/utils.ts`. Do not return to a single monolithic panel i18n file.
...
- Preserve the English product terms `Multi-master`, `Single-master`, `Master routes`, `File tasks`, `Cache`, and `Source baseline`. In Simplified Chinese, render the mode terms as `多母版模式` and `单母版模式`, and render `Source baseline` as `源文基线`.
